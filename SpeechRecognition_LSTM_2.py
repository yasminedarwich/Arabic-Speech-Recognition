...
# Updated decode_predictions function
def decode_predictions(predicted_transcriptions_encoded):
    ...
    for transcription_encoded in predicted_transcriptions_encoded:
        decoded_text = []
        for index_array in transcription_encoded:
            # Use np.argmax to find the index with the highest value
            index = np.argmax(index_array)
            if index in index_to_char:
                decoded_text.append(index_to_char[index])
        decoded_transcriptions.append(''.join(decoded_text))
